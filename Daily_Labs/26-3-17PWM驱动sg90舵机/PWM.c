...
void PWM_Init(void)
{
	...
	GPIO_InitStrusture.GPIO_Pin = GPIO_Pin_1;
	...
	TIM_TimeBaseStructure.TIM_Period = 20000-1;//周期，ARR自动重装器的值
	...
	TIM_OC2Init(TIM2, &TIM_OCInitStructure);//初始化输出比较单元
...
{
	TIM_SetCompare2(TIM2,Compare);//实时改变CCR
}
